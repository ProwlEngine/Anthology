...
{
    protected readonly long[] startRefs = { 281475006070784L };
    ...
    private readonly long[][] results =
    {
        new[]
        {
            281475006070784L, 281475006070785L, 281475005022208L, 281475005022209L, 281475003973633L,
            ...
            281474988244997L, 281474988245002L, 281474988244998L, 281474988245000L, 281474988244999L, 281474988244994L,
    ...
    public TileCacheNavigationTest()
    {
    ...
    public void TestFindPathWithDefaultHeuristic()
    {
        ...
        for (int i = 0; i < startRefs.Length; i++)
        {
            ...
            Assert.Equal(statuses[i], status);
            Assert.Equal(results[i].Length, npath);
            for (int j = 0; j < results[i].Length; j++)
            {
                Assert.Equal(results[i][j], path[j]); // TODO: @ikpil, check
            }
        }
    }
